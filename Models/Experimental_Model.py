...
class TexModel(nn.Module):
    def __init__(
        ...
        # else:
        if dataset:
            self.max_output_len =  int(self.cfg.set_max_label_length) + int(2)
        else:
            self.max_output_len = int(self.cfg.set_max_label_length) + int(2)
        ...
        self.backbone_pos = nn.Sequential(
            resnet.conv1,
            resnet.bn1,
            resnet.relu,
            resnet.maxpool,
        ...
        self.bottleneck_res = nn.Conv2d(512, self.embedding_dim, 1)  # in channels, out channels, stride
        ...
        #self.bottleneck_unet = nn.Conv2d(self.num_classes, self.embedding_dim, 1)  # in channels, out channels, stride
        ...
        transformer_decoder_layer = nn.TransformerDecoderLayer(self.embedding_dim, self.cfg.decoder_heads, self.cfg.decoder_fc, self.cfg.decoder_dropout)
        self.transformer_decoder = nn.TransformerDecoder(transformer_decoder_layer, self.cfg.decoder_layers)
    ...
    def _init_weights(self) -> None:
        ...
        nn.init.kaiming_normal_(
            self.bottleneck_res.weight.data,
            a=0,
            mode="fan_out",
            nonlinearity="relu",
        )
        if self.bottleneck_res.bias is not None:
            _, fan_out = nn.init._calculate_fan_in_and_fan_out(self.bottleneck_res.weight.data)
            bound = 1 / math.sqrt(fan_out)
            nn.init.normal_(self.bottleneck_res.bias, -bound, bound)
    ...
    def encode(self, x: Tensor) -> Tensor:
        ...
        #x = self.unet(x.float())
        x = self.backbone_pos(x.float())  # (B, RESNET_DIM, H, W); H = _H // 32, W = _W // 32
        x = self.bottleneck_res(x)  # (B, E, H, W)
